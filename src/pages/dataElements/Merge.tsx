...
import { DataElementMergeFormFields } from './merge/DataElementMergeFormFields'
import {
    DataElementMergeFormValues,
    mergeFormSchema,
    validate,
} from './merge/DataElementMergeSchema'
...
export const Component = () => {
    ...
    const onSubmit = async (values: DataElementMergeFormValues) => {
        try {
            const data = mergeFormSchema.parse(values)
            await dataEngine.mutate({
                resource: 'dataElements/merge',
                type: 'create',
                data,
            })
            return undefined
            ...
            {({ handleSubmit }) => (
                <StyledMergeForm onSubmit={handleSubmit}>
                    <DefaultMergeFormContents
                        title={
                            <Title>
                                {i18n.t('Configure data element merge')}
                            </Title>
                        }
                        mergeCompleteElement={
                            <MergeComplete>
                                <p>
                                    {i18n.t(
                                        'The data element merge operation is complete.'
                                    )}
                                </p>
                                <p>
                                    {i18n.t(
                                        'All selected data elements were merged successfully.'
                                    )}
                                </p>
                            </MergeComplete>
                        }
                    >
                        <DataElementMergeFormFields selectedIds={selectedIds} />
                    </DefaultMergeFormContents>
                </StyledMergeForm>
            )}
        </Form>
